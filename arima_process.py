...
class ArmaProcess(object):
    ...
    def from_estimation(cls, model_results, nobs=None):
        ...
        """
        if '_params_ar' in dir(model_results) and '_params_ma' in dir(model_results):
        ...
        elif '_params_ar' in dir(model_results):
            arcoefs = model_results.arparams
            nobs = nobs or model_results.nobs
            return cls(np.r_[1, -arcoefs], nobs=nobs)
        elif '_params_ma' in dir(model_results):
            macoefs = model_results.maparams
            nobs = nobs or model_results.nobs
            return cls(np.r_[1, macoefs], nobs=nobs)
